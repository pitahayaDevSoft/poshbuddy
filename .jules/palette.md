...
## 2024-10-31 - Evitar estilos de selección en mensajes de lista vacía
**Learning:** En listas renderizadas por Ratatui, insertar un mensaje de "lista vacía" como un `ListItem` ordinario causa que el estado de selección activo resalte este mensaje como si fuera interactivo. Esto rompe la navegación por teclado y confunde a los usuarios con lectores de pantalla o al navegar visualmente.
**Action:** Al renderizar listas con mensajes "vacíos" de respaldo, desactivar explícitamente los estilos de resaltado (`highlight_style` y `highlight_symbol`) usando un condicional `if !items_source.is_empty()` para mantener el mensaje como texto inactivo.
